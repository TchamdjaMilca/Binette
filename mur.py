...
import bd
from utilitaires import est_un_ami, get_utilisateur_or_die, get_utilisateur_authentifie_or_die
...
def mur(identifiant):
    ...
    with bd.creer_connexion() as conn:
        ...
        if request.method == "POST":
            ...
            if contenu:
                bd.ajouter_message(conn, get_utilisateur_authenifie_or_die()["id_utilisateur"], identifiant, contenu)
...
def afficher_amis(identifiant):
    ...
    with bd.creer_connexion() as conn:
        ...
        if "utilisateur" in session:
            ...
            id_authentifié = session["utilisateur"][0]
...
def devenir_ami(identifiant):
    """Pour devenir ami"""
    # id_authentifié = get_utilisateur_authenifie_or_die()["id_utilisateur"]
    id_authentifié = get_utilisateur_authentifie_or_die()
